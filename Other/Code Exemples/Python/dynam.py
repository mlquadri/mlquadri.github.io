"""
This is a set of function that iterate a given single varable function to find any orbits and determen the initial and periodic parts 
"""
...
def orbit_data(f,x0):
    ...
    count = 0
    fxList = [x0]
    while(True): #infinit loop if there is no cycle
        nextNum = f(x0)
        if(nextNum in fxList):
            firstApperence = fxList.index(nextNum)
            fxList.append(nextNum)
            count+=1
            return {"initial":fxList[0:firstApperence], 
                    "cycle":fxList[firstApperence:count]}
        else: #else optional
            fxList.append(nextNum)
            count+=1
            x0 = nextNum
...
def smallest_first(L):
    ...
    """
    #finds the smallest num and its index
    if(len(L) <= 0):
        return L
    smallestNum = L[0]
    smallestIndex = 0
    for i in range(len(L)):
        if(smallestNum > L[i]):
            smallestNum = L[i]
            smallestIndex = i
    #returns a array starting with the smallest
    return L[smallestIndex:]+L[0:smallestIndex]
...
def find_cycles(f,start_vals):
    ...
    for val in start_vals:
        newList = smallest_first(eventual_cycle(f,val))
        if(not(newList in cycleList)):
            cycleList.append(newList)
    return cycleList